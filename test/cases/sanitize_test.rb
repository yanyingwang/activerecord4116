...
class SanitizeTest < ActiveRecord4116::TestCase
  def setup
  end

  def test_sanitize_sql_hash_handles_associations
    quoted_bambi = ActiveRecord4116::Base.connection.quote("Bambi")
    quoted_column_name = ActiveRecord4116::Base.connection.quote_column_name("name")
    quoted_table_name = ActiveRecord4116::Base.connection.quote_table_name("adorable_animals")
  ...
  def test_sanitize_sql_array_handles_string_interpolation
    quoted_bambi = ActiveRecord4116::Base.connection.quote_string("Bambi")
    assert_equal "name=#{quoted_bambi}", Binary.send(:sanitize_sql_array, ["name=%s", "Bambi"])
    assert_equal "name=#{quoted_bambi}", Binary.send(:sanitize_sql_array, ["name=%s", "Bambi".mb_chars])
    quoted_bambi_and_thumper = ActiveRecord4116::Base.connection.quote_string("Bambi\nand\nThumper")
